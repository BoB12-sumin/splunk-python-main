...
def search_splunk(sp_qeury, file_save=False):
    ...
    try:
        ...
        server_content = requests.post(
            splunk_url + "/services/auth/login",
            data=urllib.parse.urlencode(
                {"username": SPLUNK_USERNAME, "password": SPLUNK_PASSWORD}
            ),
            verify=False,
            timeout=10,
        )
        session_key = (
            minidom.parseString(server_content.text)
        ...
        search_respone = requests.post(
            es_url + "",
            data=urllib.parse.urlencode({"search": sp_qeury}),
            headers=session_headers,
            verify=False,
            timeout=10,
        )
        sid = (
            minidom.parseString(search_respone.text)
            .getElementsByTagName("sid")[0]
        ...
        while isnotdone:
            ...
            search_respone = requests.get(
                es_url, headers=session_headers, verify=False, timeout=10
            )
...
if __name__ == "__main__":
    (
        ...
        url_result,
        filepath_result,
    ) = search_splunk('index="bob12" | head 10', file_save=True)
    print("search_result: \n\n ", search_result)
    print("json_result: \n\n ", json_result)
    print("conut_result: \n\n ", conut_result)
